...
export class Database {
	...
	public getDatabaseStats(flags, AttendeeID) {
		...
		if (listingType == "vr" || listingType == "cl" || listingType == "dl" || listingType == "cr") {	// Database version
					
			if (listingType == "vr") {	// Database version
			
				// Validate query
				SQLquery = "SELECT databaseVersion ";
				SQLquery = SQLquery + "FROM system_configuration ";
				
				console.log('Database, version query: ' + SQLquery);
					...
					this.sqlite.create({name: 'flyinPlanner.db', location: 'default', createFromLocation: 1}).then((db: SQLiteObject) => {
						...
						this.db.executeSql(SQLquery, <any>{}).then((data) => {
							...
							if(data.rows.length > 0) {
								for(let i = 0; i < data.rows.length; i++) {
									DatabaseResponse.push({
										databaseVersion: data.rows.item(i).databaseVersion
									});
								}
							}
							resolve(DatabaseResponse);
						})
						.catch(e => console.log('Database: version query error: ' + JSON.stringify(e)))
					});
					console.log('Database: Version query complete');

				});
			}

			if (listingType == "cl") {	// Close database
				...
				return new Promise(resolve => {
					...
					console.log('Database: DB close function complete');

				});
			}

			if (listingType == "dl") {	// Delete database

				// Perform query against local SQLite database
				return new Promise(resolve => {
					
					this.sqlite.deleteDatabase({name: 'flyinPlanner.db', location: 'default'}).then((db: SQLiteObject) => {
												
					});
					console.log('Database: DB delete function complete');

				});
			}

			if (listingType == "cr") {	// Create database
				...
				return new Promise(resolve => {
					...
					console.log('Database: DB create function complete');
