...
export class SearchResultsPage {
	...
	LoadData() {
		...
		this.databaseprovider.getAgendaData(flags, "0").then(data => {
			...
			console.log("SearchResults, getAgendaData: " + JSON.stringify(data));
			...
			this.databaseprovider.getCongressionalData(flags, "0").then(dataS => {
				
				console.log("SearchResults, getCongressionalData, Senate: " + JSON.stringify(dataS));

				// Process returned records to display
				this.Senators = [];

				SenatorsCount = dataS['length'];
				...
				if (dataS['length'] > 0) {
					for (var i = 0; i < dataS['length']; i++) {
						...
						DisplayName = DisplayName + dataS[i].LastName;
						
						if (dataS[i].Suffix != "" && dataS[i].Suffix != null) {
							DisplayName = DisplayName + " " + dataS[i].Suffix;
						}

						DisplayName = DisplayName + ", " + dataS[i].FirstName;
						
						if (dataS[i].MiddleInitial != "" && dataS[i].MiddleInitial != null) {
							DisplayName = DisplayName + " " + dataS[i].MiddleInitial;
						}
						
						if (dataS[i].CongressionalMember == "1") {
							visDisplayPartyState = dataS[i].Party + " - " + dataS[i].State;
						} else {
							visDisplayPartyState = "Representing: " + dataS[i].CongressionalMemberName;
						}

						this.Senators.push({
							cmID: dataS[i].congressionalMemberID,
				...
				this.databaseprovider.getCongressionalData(flags, "0").then(dataR => {
					
					console.log("SearchResults, getCongressionalData, Reps: " + JSON.stringify(dataR));

					// Process returned records to display
					this.Representatives = [];

					HouseCount = dataR['length'];
					...
					if (dataR['length'] > 0) {
						for (var i = 0; i < dataR['length']; i++) {
							...
							// Concatenate fields to build displayable name
							DisplayName = DisplayName + dataR[i].LastName;
							
							if (dataR[i].Suffix != "" && dataR[i].Suffix != null) {
								DisplayName = DisplayName + " " + dataR[i].Suffix;
							}

							DisplayName = DisplayName + ", " + dataR[i].FirstName;
							
							if (dataR[i].MiddleInitial != "" && dataR[i].MiddleInitial != null) {
								DisplayName = DisplayName + " " + dataR[i].MiddleInitial;
							}
							
							if (dataR[i].CongressionalMember == "1") {
								visDisplayPartyStateDistrict = dataR[i].Party + " - " + dataR[i].State + " - " + dataR[i].District;
							} else {
								visDisplayPartyStateDistrict = "Representing: " + dataR[i].CongressionalMemberName;
							}

							this.Representatives.push({
								cmID: dataR[i].congressionalMemberID,
								DisplayNameLastFirst: DisplayName,
